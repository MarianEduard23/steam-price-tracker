...
# Sesiune globală — păstrează cookies/headers între requests (Steam le verifică)
USER_AGENTS = [
...
try:
    SESSION.get("https://steamcommunity.com/market/", timeout=15)  # warm-up cookies
except Exception:
    pass
...
# ── Steam API ─────────────────────────────────────────────────
def parse_price_string(ps):
    """Parsează un string de preț Steam indiferent de format."""
    import re
    ps = ps.replace("€", "").replace("\xa0", "").strip()
    # elimină separatorul de mii (punct urmat de exact 3 cifre)
    ps = re.sub(r'\.(\d{3})', r'\1', ps)
    ps = ps.replace(",", ".")
    ...
    try:
        ...
        if r.status_code == 429:
            if retry < MAX_RETRIES:
                wait = 30 * (retry + 1) + random.uniform(0, 10)
                print(f"   ⏳ Rate limit! Aștept {wait:.0f}s ({retry+1}/{MAX_RETRIES})...")
                time.sleep(wait)
            ...
            if data.get("success"):
                ...
                if raw:
                    try:
                        ...
                        print(f"   ⚠️ Nu s-a putut parsa prețul '{raw}': {e}")
                        return None
            print("   ⚠️ Fără preț disponibil")
        return None
...
def main():
	...
	{'name': 'Operation Vanguard Weapon Case', 'app_id': '730', 'quantity': 10},
        {'name': 'Fever Case', 'app_id': '730', 'quantity': 530},
        {'name': 'Fracture Case', 'app_id': '730', 'quantity': 150},
        {'name': 'Kilowatt Case', 'app_id': '730', 'quantity': 60},
        {'name': 'Recoil Case', 'app_id': '730', 'quantity': 150},
        {'name': 'Revolution Case', 'app_id': '730', 'quantity': 319},
        {'name': 'Snakebite Case', 'app_id': '730', 'quantity': 150},
	{'name': 'Dreams & Nightmares Case', 'app_id': '730', 'quantity': 55},
	...
	{'name': 'Community Sticker Capsule 1', 'app_id': '730', 'quantity': 5},
        ...
        {'name': 'Sticker | Queen Ava ()', 'app_id': '730', 'quantity': 1},
        ...
        {'name': 'Sticker | Complexity Gaming () | Austin 2025', 'app_id': '730', 'quantity': 3},
        ...
        {'name': 'Sticker | M80 () | Austin 2025', 'app_id': '730', 'quantity': 10},
        {'name': 'Sticker | FaZe Clan () | Austin 2025', 'app_id': '730', 'quantity': 20},
        {'name': 'Sticker | iM (Foil) | Austin 2025', 'app_id': '730', 'quantity': 185},
