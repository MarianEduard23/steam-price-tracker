...
MAX_RETRIES         = 4
DELAY_BETWEEN_REQS  = 3             # secunde între request-uri
    ...
    try:
        ...
        if r.status_code == 429:
            if retry < MAX_RETRIES:
                wait = 20 * (retry + 1) + random.uniform(0, 5)
        ...
        # Filtrăm listingurile care nu au câmpul "price"
        buy_now = [l for l in buy_now if l.get("price") is not None]
        if not buy_now:
            print("   ⚠️ Niciun listing cu preț valid pe CSFloat")
            return None
...
def create_excel(items_list: list, output: str = "csfloat_items.xlsx"):
    ...
    for idx, item in enumerate(items_list, 1):
        ...
        if idx < len(items_list):
            wait = DELAY_BETWEEN_REQS + random.uniform(0, 2)
            time.sleep(wait)
